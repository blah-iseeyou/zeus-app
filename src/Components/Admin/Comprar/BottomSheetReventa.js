...
import { Modal, Keyboard, Dimensions, ScrollView, Linking } from 'react-native';
...
export default function (props) {
    ...
    const { reventa_id, hacienda_id, onClose } = props
    ...
    const [reventaId, setReventaId] = useState()
    ...
    let getHacienda = () => {
        ...
        axios.get('/hacienda', {
            ...
            .then(response => {
                console.log("HACIENDA", response.data.data)
            ...
            .catch(error => {
                return toast.show({
                    duration: 2500,
                    render: () => {
                        return <Box bg="red.500" px="2" py="1" rounded="sm" color="white" mb={5}>No fue posible obtener la hacienda</Box>;
                    },
                    top: 10
                })
            })
    ...
    let getContratoInfo = ({plantas = plantas, moneda = moneda} = {}) => {
        setMoneda(moneda)
        setPlantas(plantas)
    ...
    let pagoTarjeta = () => {
        ...
        axios.post('/authorizenet/pago', {
            ...
            .catch(error => {
                ...
                setError(error?.response?.data?.descripcion ?? "Hubo un error al pagar")
                // return toast.show({
                //     duration: 2500,
                //     render: () => {
                //         return <Box bg="red.500" px="5" py="1" rounded="sm" zIndex={1000}>
                //             <Heading color="white" fontSize="sm">{ ?? "Hubo un error al pagar"}</Heading>
                //         </Box>;
                //     },
                //     top: 0
                // })
    ...
    if (reventa_id && reventa_id != null && reventa_id !== reventaId) {
        getHacienda()
        getReventa(reventa_id)
        console.log("REVENTA")
            ...
            <KeyboardAvoidingView height="100%" behavior='height'>
                <ScrollView
                    ...
                    contentContainerStyle={{
                        ...
                        paddingBottom: (keyboardStatus) ? 0 : 0,
                    }}
                >
                    <Box flex={1} px={3} py={3}>
                        <Button rounded={100} h={7} w={7} position="absolute" zIndex={100} right={2} top={2} bg="red.500" startIcon={<Icon as={AntDesign} name="close" />} onPress={() => {
                            onClose()
                            bottomSheetRef?.current?.close()
                            setReventaId()
                            restart()
                        ...
                        {
                            !compra ? <>
                                {!keyboardStatus && <Box flex={0.5} />}
                                <FormControl isInvalid={plantas > reventa?.cantidad_restante || plantas < 1}>
                                    <FormControl.Label>Cantidad de Plantas ({reventa?.cantidad_restante} Max)</FormControl.Label>
                                    <InputFormatNumber
                                        value={plantas}
                                    ...
                                    <FormControl.ErrorMessage>
                                        {
                                            plantas > reventa?.cantidad_restante && "Excedes las plantas disponibles"
                                        }
                                        {
                                            plantas < 1 && "El minimo es 1 planta"
                                        }
                                    </FormControl.ErrorMessage>
                                </FormControl>
                                {!keyboardStatus && <Box flex={0.2} />}
                                {/* <Box flex={0.2} /> */}
                                <FormControl isInvalid mt={5} >
                                    ...
                                    <Button.Group isAttached mx={{ base: "auto", md: 0 }} w="100%">
                                        <Button size="sm" flex={1}
                                            onPress={() => getContratoInfo({plantas, moneda: "MXN"})}
                                        ...
                                        <Button
                                            onPress={() => getContratoInfo({plantas, moneda: "USD"})}
                                ...
                                <Box>
                                    <Button
                                        isLoading={loading}
                                        isDisabled={(!plantas || plantas < 1 || plantas > reventa.cantidad_restante) || !moneda}
                                        onPress={() => {
                                            if (contratoAceptado) {
                                                comprarPlantas()
                                            } else {
                                ...
                                {(moneda == "USD") && <FormControl isInvalid mt={5}>
                                    <Button.Group isAttached mx={{ base: "auto", md: 0 }} w="100%">
                                        <Button
                                            size="sm"
                                            flex={1}
                                ...
                                {(!metodoPago && (moneda == "USD")) ? <>
                                    ...
                                    <Box flex={1} mb={3} />
                                    <Button isLoading={loading} onPress={pagoTarjeta}>Pagar</Button>
                                    ...
                                    }} startIcon={<Icon viewBox="0 0 30 32" size={"xl"} mx="auto">
                                    ...
                                    </Icon>}>Descargar la ficha de pago</Button>
                                </>}
                            </>
                        }
                    </Box>
